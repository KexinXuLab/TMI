...
def extract(matrix):
    ...
    for i in range(2, m - 2):
        for j in range(2, n - 2):
            Extracting_elements.append(
                [matrix[i - 2, j - 2], matrix[i - 2, j - 1], matrix[i - 2, j], matrix[i - 2, j + 1],
                 matrix[i - 2, j + 2],
                 matrix[i - 1, j - 2], matrix[i - 1, j - 1], matrix[i - 1, j], matrix[i - 1, j + 1],
                 matrix[i - 1, j + 2],
                 matrix[i, j - 2], matrix[i, j - 1], matrix[i, j], matrix[i, j + 1], matrix[i, j + 2],
                 matrix[i + 1, j - 2], matrix[i + 1, j - 1], matrix[i + 1, j], matrix[i + 1, j + 1],
                 matrix[i + 1, j + 2],
                 matrix[i + 2, j - 2], matrix[i + 2, j - 1], matrix[i + 2, j], matrix[i + 2, j + 1],
                 matrix[i + 2, j + 2]])
...
if __name__ == '__main__':
    ...
    for m in range(100):
        ...
        print(TP, FP, TN, FN, precision, recall, f1, specificity, accuracy, np.max(ML_count), np.mean(ML_count), np.std(ML_count, ddof=1))
